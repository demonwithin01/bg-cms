...
module outofmars
{
    class Light
    {
        ...
        private _background: PageBackground;
        private _direction: number;
        private _blurRadius: number;
        private _flickerRate: number;
        private _minimumBlurRadius: number;
        private _maximumBlurRadius: number;
        private _timeTillNextFlicker: number;
        private _timeBetweenFlickers: number;
        private _drawInterval: number;

        constructor( background: PageBackground, x: number, y: number, size: number, life: number, drawInterval: number )
        {
        ...
        public draw( context: CanvasRenderingContext2D )
        {
            ...
            if ( this._timeTillNextFlicker < 0 )
            {
                ...
                if ( t > this._flickerRate ) p = 200 - p;
    ...
    export class PageBackground //extends site.plugin
    {
        private _canvas: HTMLCanvasElement;
        private _context: CanvasRenderingContext2D;
        private _lights: Array<Light>;
        private _drawInterval: number;
        private _timeBetweenRenders: number;
